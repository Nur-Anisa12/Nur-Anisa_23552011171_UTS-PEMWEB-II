...
<head>
    ...
    <title>Edit Mata Kuliah - Nura Akademik</title>
        ...
        <main class="flex-1 flex flex-col min-w-0">
            ...
            <header class="topbar h-14 flex items-center justify-between px-6 flex-shrink-0">
                <div class="flex items-center gap-2 text-[13px] text-[#9aa5b4]">
                    ...
                    <span class="text-[#3a4a5c] font-medium">Edit Data</span>
                </div>
                ...
                <div class="max-w-md w-full">
                    <div class="main-card shadow-sm overflow-hidden">
                        <div class="p-6 border-b border-[#f0f2f5] bg-[#fcfdfe]">
                            <h1 class="text-[18px] font-bold text-[#1a2540]">Edit Mata Kuliah</h1>
                            <p class="text-[13px] text-[#8a96a6] mt-0.5">Perbarui informasi kurikulum mata kuliah.</p>
                        </div>
                        ...
                        <form action="{{ route('matakuliah.update', $matakuliah->id) }}" method="POST"
                            ...
                            {{-- Input Nama MK --}}
                            <div>
                                <label class="block text-[13px] font-bold text-[#3a4a5c] mb-2">Nama Mata Kuliah</label>
                                <div class="relative">
                                    <i
                                        class="ph ph-book-open absolute left-4 top-1/2 -translate-y-1/2 text-[#9aa5b4]"></i>
                            ...
                            {{-- Input SKS --}}
                            <div>
                                <label class="block text-[13px] font-bold text-[#3a4a5c] mb-2">Jumlah SKS</label>
                                <div class="relative">
                                    <i class="ph ph-hash absolute left-4 top-1/2 -translate-y-1/2 text-[#9aa5b4]"></i>
                                    <input type="number" name="sks" value="{{ $matakuliah->sks }}" min="1"
                                        max="6" required
                                        class="input-field w-full pl-11 pr-4 py-3 rounded-xl text-[14px] outline-none"
                                        placeholder="1 - 6">
                                </div>
                            ...
                            {{-- Select Jurusan --}}
                            <div>
                                <label class="block text-[13px] font-bold text-[#3a4a5c] mb-2">Pilih Jurusan</label>
                                <div class="relative">
                                    <i
                                    ...
                                    <select name="id_jurusan" required
                                        ...
                                        @foreach ($jurusan as $j)
                                            <option value="{{ $j->id }}"
                                                {{ $matakuliah->id_jurusan == $j->id ? 'selected' : '' }}>
                                                {{ $j->nama_jurusan }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="flex gap-3 pt-2">
                                <a href="/matakuliah"
                                    class="flex-1 bg-slate-100 hover:bg-slate-200 text-[#3a4a5c] text-center font-bold py-3.5 rounded-xl text-[14px] transition-all">
                                    Batal
                                </a>
                                <button type="submit"
                                    class="flex-1 bg-[#1a4fad] hover:bg-[#154191] text-white font-bold py-3.5 rounded-xl shadow-md flex items-center justify-center gap-2 text-[14px] transition-all transform active:scale-[0.98]">
